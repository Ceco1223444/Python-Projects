...
def longest_common_subsequence(X,Y):
    ...
    # 1. Сменяме np.zeros с чист Python списък. 
    # Това е единствената голяма промяна, която ти трябва за скорост.
    dp = [[0] * (n + 1) for _ in range(m + 1)] 
    for i in range(1, m + 1):
        # 2. Кешираме символа от X, за да не го викаме 1000 пъти в подцикъла
        current_x = X[i-1] 
        ...
        for j in range(1, n + 1):
            if current_x == Y[j-1]:
                ...
                # 3. Ползваме твоя max, но върху чистия списък е 10 пъти по-бързо
                val1 = dp[i-1][j]
                val2 = dp[i][j-1]
